/* ==========================================================================
   Comparely — Compare page logic
   Depends on: products.js (PRODUCTS) and common.js (ICONS, CATEGORY_ICONS,
   CATEGORY_GRADIENTS, CATEGORIES, getProduct, fmtPrice, fmtCount, starsHTML,
   raf, safeScrollIntoView, showToast). Load order: products.js -> common.js
   -> compare.js (see compare.html).
   ========================================================================== */
...
function renderProductGrid() {
  ...
  els.productGrid.innerHTML = shown.map(function (p, i) { return productCardHTML(p, i); }).join('');

  const remaining = list.length - shown.length;
  if (remaining > 0) {
    els.showMoreBtn.hidden = false;
    els.showMoreBtn.textContent = 'Show ' + Math.min(remaining, PAGE_SIZE) + ' more (' + remaining + ' left)';
  } else {
...
/* ---------- URL params: ?q=, ?category=, ?a=&b= (from Home / nav search) ---------- */
function applyUrlParams() {
  ...
  if (aId && getProduct(Number(aId))) {
    state.selected[0] = Number(aId);
    state.selected[1] = (bId && getProduct(Number(bId))) ? Number(bId) : null;
  } else if (bId && getProduct(Number(bId))) {
    state.selected[1] = Number(bId);
  }
}
...
function init() {
  if (typeof PRODUCTS === 'undefined' || !Array.isArray(PRODUCTS)) {
    console.error('Comparely: PRODUCTS is not defined. Make sure products.js loads before compare.js.');
    return;
  }
  applyUrlParams();
  renderCategoryPills();
